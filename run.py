...
# ── Giải phóng port: kill thẳng process mitmdump qua /proc/*/cmdline ────────

def _free_port(_port: int):
    """Kill tất cả process mitmdump — không dùng lệnh ngoài, không cần /proc/net."""
    my_pid = os.getpid()
    killed = False
    try:
        for entry in os.listdir("/proc"):
            ...
            try:
                ...
                if "mitmdump" in cmdline:
                    ...
                    print(f"[mitm] Đã SIGKILL mitmdump PID {pid}")
                    killed = True
            except (FileNotFoundError, ProcessLookupError, PermissionError):
                pass
    except Exception as e:
        print(f"[mitm] _free_port lỗi: {e}")
