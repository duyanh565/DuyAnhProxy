...
def _wait_port_free(port: int, timeout: int = 60) -> bool:
...
def run_mitmproxy():
    ...
    while True:
        print(f"[mitm] Chờ port {PROXY_PORT} free...")
        if not _wait_port_free(PROXY_PORT, timeout=60):
            print(f"[mitm] Port {PROXY_PORT} vẫn bận sau 60s — thử khởi động thẳng...")
        ...
        try:
            ...
            # Đảm bảo kill toàn bộ process group khi thoát
            if proc is not None:
                try:
